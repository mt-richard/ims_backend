...
exports.getStockMovementByItem = async (filters) => {
  try {
    const { assetId, location, startDate, endDate } = filters;
    ...
    const combinedResults = [
      ...
      ...stockAdjustments.map(adjustment => ({
        ...adjustment.toJSON(),
        type: 'stock_adjustment',
        // item_id: adjustment.itemId.item_id,
        // asset_id: adjustment.itemId.asset_id,
        item_name: adjustment.itemId.item_name,
        unit: adjustment.itemId.unit,
        location_name: adjustment.Location.location_name,
        division_name: adjustment.divisionId.division_name,
    ...
    throw new Error(`Error fetching stock movements by item: ${error.message}`);
  }
};