...
exports.createPurchaseEntry = async (data) => {
  ...
  try {
    ...
    // Generate ONE ref_no for all items
    const lastEntry = await purchase_entry.findOne({
      order: [["purchase_id", "DESC"]],
      attributes: ["ref_no"],
      transaction,
    ...
    for (const item of items) {
      ...
      const purchaseQty = parseInt(item.quantity, 10);
      ...
      // ✅ PER ITEM LOGIC
      if (item.purchase_type === "import") {
        const fxRate = parseFloat(item.fx_rate);
        const fxAmount = parseFloat(item.fx_amount);

        if (!fxRate || !fxAmount) {
          throw new Error(`Invalid FX data for item ${item.item_id}`);
        }

        unitPrice = fxRate * fxAmount;
      } else {
      ...
      if (item.doc_type === "GRN") {
        ...
        const locationIndex = locationData.findIndex(
          (loc) => loc.location_id === parseInt(item.received_location, 10)
        );
        ...
        if (locationIndex !== -1) {
          ...
          locationData.push({
            location_id: parseInt(item.received_location, 10),
            qty: purchaseQty,
          });
        }
        ...
        await item_master.update(
          {
            ...
            location: locationData,
            cost: totalPrice,
            bin_location: item.received_location,
      ...
      const purchaseEntry = await purchase_entry.create(
        {
          ...
          supp_invoice_no: header.supp_invoice_no,
      ...
      createdEntries.push(purchaseEntry);
    }

    await transaction.commit();

    return {
      message: "Purchase Entries recorded successfully",
      items: createdEntries,
    };

  } catch (error) {
    await transaction.rollback();
    throw new Error(`Error creating purchase entries: ${error.message}`);
  }
};